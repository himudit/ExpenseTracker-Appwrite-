...
import assistantBg from '../../../public/assistanc_bg.png'
...
const Assistant = () => {
    ...
    return (
        <div
            className="flex flex-col h-screen md:ml-[4rem] text-gray-800 font-sans overflow-hidden relative selection:bg-[#033ff6]/30 selection:text-indigo-900"
            style={{
                backgroundImage: `url(${assistantBg})`,
        ...
        >
            ...
            <div className={`flex-1 flex flex-col w-full mx-auto px-4 overflow-hidden relative z-10 h-full ${hasMessages ? '' : 'justify-center'}`}>
                ...
                {/* Messages Area - full-width scroll wrapper so scrollbar is at page edge */}
                    ...
                    <div className={`w-[50%] max-w-4xl mx-auto flex flex-col ${hasMessages ? 'pt-8 pb-4 space-y-8' : ''}`}>
                        {messages.map((msg, idx) => (
                            <div key={idx} className={`flex gap-4 md:gap-6 ${msg.role === 'user' ? 'justify-end' : 'justify-start'} group`}>
                                ...
                                {msg.role === 'user' && (
                                    <div className="w-7 h-7 md:w-10 md:h-10 flex-shrink-0 rounded-md bg-gray-200 flex items-center justify-center border border-gray-300 mt-1 shadow-sm overflow-hidden ">
                ...
                <div className={`w-[60%] transition-all duration-700 ease-in-out mx-auto flex-shrink-0 ${!hasMessages ? 'max-w-3xl' : 'max-w-4xl'} pb-6 pt-2`}>
                    <div className="relative flex items-center w-full bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-200 transition-colors overflow-hidden focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-300">
                        <textarea
                            ...
                            className="w-full max-h-30 py-3 pl-4 pr-14 bg-transparent text-gray-800 placeholder-gray-400 resize-none outline-none overflow-y-auto text-base leading-relaxed"
                            rows="1"
